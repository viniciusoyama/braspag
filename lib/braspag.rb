...
module Braspag
  ...
  QUERY_WSDL = {
    :homologation => "https://homologacao.pagador.com.br/services/pagadorQuery.asmx?WSDL",
    :production => "https://transaction.pagador.com.br/services/pagadorQuery.asmx?WSDL"
  }

  class << self
    attr_accessor :merchant_id, :merchant_key, :logging, :steps_logger, :production, :payment_data_request_xml_builder
    ...
    def query_wsdl
      production? ? QUERY_WSDL[:production] : QUERY_WSDL[:homologation]
    end
...
Braspag.configure do |config|
  ...
  config.steps_logger = Logger.new("/dev/null")
